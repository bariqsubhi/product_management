      ...
      && category.value != ""){
         ...
         if(btnMood === 'create'){
            ...
            if(newProduct.count > 0 && newProduct.count <= 100){
               ...
               alert("رجاءا، تاكد من ادخال رقم عدد صالح (1-100)");
            }
         ...
         alert("رجاءا تاكد من ملئ جميع الحقول");
      }
...
function showData(){
   ...
   if(dataProduct.length > 0){
      btnDeleteAll.innerHTML = ` <button onclick="deleteAll()">حذف الكل (${dataProduct.length})</button> ` ;
   }else{
...
function getSearchMood(btnId){
   ...
   search.placeholder = 'البحث ب' + serchType;
   search.focus();
   search.value = '';
   showData();
}
